<template>
	<view class="dj-detail">
		...
		<view class="detail-content">
			...
			<!-- 详情 -->
			<block v-if="isDetail">
				<detail :avatarUrl='avatarUrl' :nickname="nickname" :category='category' :desc="desc"></detail>
			</block>
...
<script>
	import {getDetail,getProgram} from '../../service/dj.js'
	...
	export default{
		...
		onLoad(options) {
			const djId=options.id
			this._getDetail(djId)
			//把图片路径中的 # 号替换成 = 号
			const img=options.img
			this.bgImg=img.replace(/#/g,'=')
			this.subCount=options.subcount
			//  电台节目 rid
		...
		methods:{
			...
			_getDetail(id){
				getDetail(id).then(res=>{
					console.log(res)
					this.avatarUrl=res.data.program.dj.avatarUrl
					this.category=res.data.program.radio.category
					this.desc=res.data.program.radio.desc
					this.name=res.data.program.dj.brand
					console.log(this.name,'dddddddddd')
					this.nickname=res.data.program.dj.nickname
				})
			},
			_getProgram(rid){
